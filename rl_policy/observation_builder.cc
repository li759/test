...
SwiftObservation SwiftObservationBuilder::BuildObservationFromObstacles(
  ...
  return BuildObservationWithParams(
      empty_point_cloud, vehicle_state, obstacles, target_position, target_yaw,
      reference_line, kDefaultLidarMaxRange, kDefaultImgViewRange);
}
...
SwiftObservation SwiftObservationBuilder::BuildObservationWithParams(
    ...
    const swift::common::math::Vec2d &target_position, double target_yaw,
    const std::shared_ptr<swift::planning::ReferenceLine> &reference_line,
    double lidar_max_range, double img_view_range) {
  ...
  auto target_info = target_extractor_.ExtractTargetInfo(
      vehicle_state, target_position, target_yaw);
...
bool ObservationBuilder::ValidateObservation(
  ...
  auto has_invalid_value = [](const std::vector<float> &vec) {
    return std::any_of(vec.begin(), vec.end(), [](float val) {
      return std::isnan(val) || std::isinf(val);
    });
  };

  if (has_invalid_value(observation.lidar) ||
      has_invalid_value(observation.target) ||
      has_invalid_value(observation.img) ||
      has_invalid_value(observation.action_mask) ||
...
ObservationBuilder::GetObservationStats(const SwiftObservation &observation) {
  ...
  // Lidar statistics
  auto lidar_sum =
      std::accumulate(observation.lidar.begin(), observation.lidar.end(), 0.0f);
  auto lidar_min =
      *std::min_element(observation.lidar.begin(), observation.lidar.end());
  auto lidar_max =
      *std::max_element(observation.lidar.begin(), observation.lidar.end());

  // Target statistics
  auto target_sum = std::accumulate(observation.target.begin(),
                                    observation.target.end(), 0.0f);

  // Image statistics
  auto img_sum =
      std::accumulate(observation.img.begin(), observation.img.end(), 0.0f);
  auto img_nonzero =
      std::count_if(observation.img.begin(), observation.img.end(),
                    [](float val) { return val > 0.0f; });

  // Action mask statistics
  auto action_mask_sum = std::accumulate(observation.action_mask.begin(),
                                         observation.action_mask.end(), 0.0f);
  auto action_mask_available = std::count_if(
      observation.action_mask.begin(), observation.action_mask.end(),
      [](float val) { return val > 0.5f; });

  ss << "SwiftObservation Stats:\n";
  ss << "  Lidar: sum=" << lidar_sum << ", min=" << lidar_min
     << ", max=" << lidar_max << "\n";
  ss << "  Target: sum=" << target_sum << "\n";
  ss << "  Image: sum=" << img_sum << ", nonzero_pixels=" << img_nonzero
     << "\n";
  ss << "  ActionMask: sum=" << action_mask_sum
     << ", available_actions=" << action_mask_available << "\n";
