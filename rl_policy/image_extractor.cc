...
std::vector<float> ImageExtractor::ExtractOccupancyGrid(
    ...
    int channels,
    double view_range) {
  std::cout << "[RL] =====================================ExtractOccupancyGrid============================="
            << std ::endl;

  double resolution = view_range / std::max(width, height);
  return ExtractOccupancyGridWithResolution(vehicle_state, obstacles, width, height, channels, view_range, resolution);
}
...
std::vector<float> ImageExtractor::ExtractOccupancyGridWithResolution(
  ...
  for (const auto& obstacle : obstacles) {
    RasterizeObstacle(obstacle, grid, width, height, resolution, vehicle_x, vehicle_y, vehicle_yaw);
  }

  return grid;
}

std::vector<float> ImageExtractor::CreateEmptyGrid(int width, int height, int channels) {
...
void ImageExtractor::RasterizeObstacle(
  ...
  RasterizeBoundingBox(bounding_box, grid, width, height, resolution, vehicle_x, vehicle_y, vehicle_yaw);
}

void ImageExtractor::RasterizeBoundingBox(
    const swift::common::math::Box2d& bounding_box,
    std::vector<float>& grid,
    int width,
    int height,
    double resolution,
  ...
  for (const auto& corner : corners) {
    auto grid_coord = WorldToGrid(corner.x(), corner.y(), vehicle_x, vehicle_y, vehicle_yaw, resolution, width, height);

    if (IsValidGridCoordinate(grid_coord.first, grid_coord.second, width, height)) {
  ...
  for (int y = min_grid_y; y <= max_grid_y; ++y) {
    for (int x = min_grid_x; x <= max_grid_x; ++x) {
      ...
      if (bounding_box.IsPointIn(swift::common::math::Vec2d(rotated_x, rotated_y))) {
...
std::pair<int, int> ImageExtractor::WorldToGrid(
  ...
  int grid_x = static_cast<int>(std::round(local_x / resolution + grid_width / 2));
  int grid_y = static_cast<int>(std::round(local_y / resolution + grid_height / 2));

  return std::make_pair(grid_x, grid_y);
}

bool ImageExtractor::IsValidGridCoordinate(int grid_x, int grid_y, int width, int height) {
