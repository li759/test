...
std::vector<float> ActionMaskExtractor::ExtractActionMask(
  ...
  return ExtractActionMaskWithCustomActions(vehicle_state, obstacles, steering_actions, step_lengths, reference_line);
}
...
std::vector<float> ActionMaskExtractor::ExtractActionMaskWithCustomActions(
  ...
  for (double steering : steering_actions) {
    for (double step_length : step_lengths) {
      bool is_valid = IsActionValid(vehicle_state, obstacles, steering, step_length, reference_line);
...
std::vector<float> ActionMaskExtractor::CreateDefaultActionMask(int num_actions) {
  return std::vector<float>(num_actions, 1.0f);
}

std::vector<float> ActionMaskExtractor::ExtractActionMaskFromLidar(const std::vector<float>& raw_lidar_obs) {
  // Initialize if not done yet
  std::cout << "[RL] =====================================ExtractActionMaskFromLidar============================="
            << std ::endl;
  if (!initialized_) {
    InitVehicleBox();
  ...
  std::vector<std::vector<std::vector<double>>> step_save(
      kHopeLidarNum, std::vector<std::vector<double>>(kHopeNumActions, std::vector<double>(kHopeNIter, 0.0)));
...
bool ActionMaskExtractor::IsActionValid(
  ...
  if (reference_line) {
    auto predicted_state = PredictVehicleState(vehicle_state, steering_angle, step_length);
...
swift::common::VehicleState ActionMaskExtractor::PredictVehicleState(
    const swift::common::VehicleState& vehicle_state, double steering_angle, double step_length) {
  ...
  double new_yaw = current_yaw + (current_speed / wheelbase) * std::sin(beta) * dt;
...
void ActionMaskExtractor::GetDefaultActionSpace(
  ...
  for (int i = 0; i < kDefaultNumSteeringActions; ++i) {
    double steering =
        kDefaultMinSteering + (kDefaultMaxSteering - kDefaultMinSteering) * i / (kDefaultNumSteeringActions - 1);
  ...
  for (int i = 0; i < kDefaultNumStepLengths; ++i) {
    double step_length =
        kDefaultMinStepLength + (kDefaultMaxStepLength - kDefaultMinStepLength) * i / (kDefaultNumStepLengths - 1);
...
std::vector<double> ActionMaskExtractor::LinearInterpolate(const std::vector<double>& x, int upsample_rate) {
...
std::vector<float> ActionMaskExtractor::PostProcessStepLen(const std::vector<float>& step_len) {
  ...
  for (int i = 0; i < static_cast<int>(result.size()); ++i) {
    ...
    for (int j = std::max(0, i - half_kernel); j <= std::min(static_cast<int>(result.size()) - 1, i + half_kernel);
         ++j) {
...
std::vector<std::vector<double>> ActionMaskExtractor::Intersect(
    const std::vector<std::vector<double>>& e1, const std::vector<std::vector<double>>& e2) {
