...
// Optional bag replay headers (guarded to avoid hard dependency)
#if __has_include("core/meta/record/record_reader.h") && __has_include("core/meta/messaging.h")
#include "core/meta/record/record_reader.h"
#include "core/meta/messaging.h"
#if __has_include("modules/planning/message_preprocessor/common_helper.h")
#include "modules/planning/message_preprocessor/common_helper.h"
#define OBS_BUILDER_HAS_PRED 1
#endif
#define OBS_BUILDER_HAS_BAG 1
#endif
...
// Closer to planning_main_replay.cc: stepwise fetch multiple topics then build
TEST_F(ObservationBuilderTest, BuildObservation_ReplayStyleMultiInputs) {
  const char* bag_path = std::getenv("SWIFT_TEST_BAG");
  if (!bag_path) {
    GTEST_SKIP() << "Set SWIFT_TEST_BAG to a valid .bag path to run this test.";
  }
  meta_bag::RecordReader reader;
  if (!reader.Open(bag_path)) {
    GTEST_SKIP() << "Failed to open bag: " << bag_path;
  }

  swift_meta::FlatArrayMessage flat;
  meta_bag::RecordMessage message;

  bool traffic_light_ready = false;
  bool routing_ready = false;
  bool chassis_ready = false;
  bool localization_ready = false;
  bool light_map_ready = false;
  bool prediction_ready = false;
  bool freespace_ready = false;

  swift::common::VehicleState vehicle_state; // default if not parsed
  std::vector<swift::planning::Obstacle> obstacles; // from prediction
  // parking slot container filled from light_map
  ParkingSlot parking_slot;

  int guard_loops = 20000;
  while (guard_loops-- > 0 && reader.ReadMessage(&message)) {
    const auto& ch = message.channel_name;
    if (ch == std::string("/swift/perception/traffic_light")) {
      (void)flat; traffic_light_ready = true;
    } else if (ch == std::string("/swift/routing_response")) {
      (void)flat; routing_ready = true;
    } else if (ch == std::string("/swift/canbus/chassis")) {
      (void)flat; chassis_ready = true;
    } else if (ch == std::string("/swift/localization/pose")) {
      /******************************************************************************
 * Copyright 2024 Desay SV Automotive Co., Ltd.
 * Copyright 2018 The Apollo Authors. All Rights Reserved.
 *
...
TEST_F(ObservationBuilderTest, BuildObservation_ReplayStyleMultiInputs) {
  ...
  while (guard_loops-- > 0 && reader.ReadMessage(&message)) {
    ...
    if (ch == std::string("/swift/perception/traffic_light")) {
      ...
      try {
        kj::ArrayPtr<const capnp::word> words(reinterpret_cast<const capnp::word*>(message.msg_data.data()),
                                              message.msg_data.size() / sizeof(capnp::word));
        capnp::FlatArrayMessageReader readerMsg(words);
        auto loc = readerMsg.getRoot<swift_messages::localization::LocalizationEstimate>();
        // pose: (x,y,theta) in world frame
        auto pose = loc.getPose();
        vehicle_state.set_x(pose.getPosition().getX());
        vehicle_state.set_y(pose.getPosition().getY());
        vehicle_state.set_heading(pose.getHeading());
      } catch (...) {
        // fallback to non-zero defaults
        vehicle_state.set_x(241480.0);
        vehicle_state.set_y(2543770.0);
        vehicle_state.set_heading(0.123);
      }
      localization_ready = true;
    } else if (ch == std::string("/swift/map/map")) {
      light_map_ready = true;
      try {
        kj::ArrayPtr<const capnp::word> words(reinterpret_cast<const capnp::word*>(message.msg_data.data()),
                                              message.msg_data.size() / sizeof(capnp::word));
        capnp::FlatArrayMessageReader readerMsg(words);
        auto mapNew = readerMsg.getRoot<swift_messages::Event>().getMapNew();
        auto map = mapNew.getMap();
        auto slots = map.getParkingSlot();
        if (slots.size() > 0) {
          auto s = slots[0];
          auto pts = s.getPt();  // 8 points: corner-mid alternating
          if (pts.size() >= 8) {
            auto getV2 = [&](int idx) {
              return swift::common::math::Vec2d(pts[idx].getX(), pts[idx].getY());
            };
            // corners at indices 0,2,4,6
            parking_slot.p0 = getV2(0);
            parking_slot.p1 = getV2(2);
            parking_slot.p2 = getV2(4);
            parking_slot.p3 = getV2(6);
            parking_slot.angle = std::atan2(parking_slot.p1.y() - parking_slot.p0.y(),
                                            parking_slot.p1.x() - parking_slot.p0.x());
            parking_slot.width = parking_slot.p0.DistanceTo(parking_slot.p1);
          }
        }
      } catch (...) {
        // keep default if parse fails
      }
    } else if (ch == std::string("/swift/prediction")) {
      prediction_ready = true;
      // Decode capnp PredictionObstacles and convert to proto
      try {
        kj::ArrayPtr<const capnp::word> words(reinterpret_cast<const capnp::word*>(message.msg_data.data()),
                                              message.msg_data.size() / sizeof(capnp::word));
        capnp::FlatArrayMessageReader readerMsg(words);
        auto predReader = readerMsg.getRoot<swift_messages::prediction::PredictionObstacles>();
        auto proto = swift::planning::CommonHelper::PredictionObstaclesFromMessage(predReader);
        // Build planning obstacles from proto
        auto list = swift::planning::Obstacle::CreateObstacles(proto);
        obstacles.clear();
        for (auto& up : list) {
          obstacles.push_back(*up);
        }
      } catch (...) {
        // keep empty on failure
      }
    } else if (ch == std::string("/swift/perception/fusion/freespace")) {
      (void)flat; freespace_ready = true;
    }

    // Minimal set to proceed (align with replay main): map + loc + chassis + prediction + freespace
    if (light_map_ready && localization_ready && chassis_ready && prediction_ready && freespace_ready) {
      // Build immediately after ready
      ParkingSlot parking_slot;
      parking_slot.p0 = swift::common::math::Vec2d(5.0, 2.0);
      parking_slot.p1 = swift::common::math::Vec2d(5.0, -2.0);
      parking_slot.p2 = swift::common::math::Vec2d(8.0, -2.0);
      ...
      // use obstacles parsed from prediction if available
      swift::perception::base::PointDCloud empty_cloud;

      ObservationBuilder builder;
      auto obs = builder.BuildObservationFromParkingSlot(
          empty_cloud, vehicle_state, obstacles, parking_slot,
          nullptr, 10.0, 20.0, false);

      EXPECT_TRUE(builder.ValidateObservation(obs));
      EXPECT_EQ(obs.lidar.size(), SwiftObservation::GetLidarDim());
      EXPECT_EQ(obs.target.size(), SwiftObservation::GetTargetDim());
      EXPECT_EQ(obs.img.size(), SwiftObservation::GetImgDim());
      EXPECT_EQ(obs.action_mask.size(), SwiftObservation::GetActionMaskDim());
      return;  // test done
    }
  }

  if (!(light_map_ready && localization_ready && chassis_ready && prediction_ready && freespace_ready)) {
    GTEST_SKIP() << "Not all required topics observed within scan window.";
  }
}
#endif  // OBS_BUILDER_HAS_BAG
