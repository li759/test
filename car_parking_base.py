...
from configs import *
from env.gear_shift_reward import GearShiftRewardCalculator
...
class CarParking(gym.Env):
    ...
    def __init__(self, render_mode: str=None, fps: int=FPS, verbose: bool=True, use_lidar_observation: bool=USE_LIDAR, use_img_observation: bool=USE_IMG, use_action_mask: bool=USE_ACTION_MASK):
        super().__init__()
        ...
        # 新增: 档位奖励计算器
        self.gear_shift_calc = GearShiftRewardCalculator()
        ...
        self.current_plot_ax = None  # 初始化实例的图表对象
        self.action_space = spaces.Box(np.array([VALID_STEER[0], VALID_SPEED[0]]).astype(np.float32), np.array([VALID_STEER[1], VALID_SPEED[1]]).astype(np.float32))
        self.observation_space = {}
        if self.use_action_mask:
            self.action_filter = ActionMask()
            self.observation_space['action_mask'] = spaces.Box(low=0, high=1, shape=(N_DISCRETE_ACTION,), dtype=np.float64)
        if self.use_img_observation:
            self.img_processor = Obs_Processor()
            self.observation_space['img'] = spaces.Box(low=0, high=255, shape=(OBS_W // self.img_processor.downsample_rate, OBS_H // self.img_processor.downsample_rate, self.img_processor.n_channels), dtype=np.uint8)
        ...
        if self.use_lidar_observation:
            ...
            self.observation_space['lidar'] = spaces.Box(low=low_bound, high=high_bound, shape=(LIDAR_NUM,), dtype=np.float64)
        low_bound = np.array([0, -1, -1, -1, -1])
        high_bound = np.array([MAX_DIST_TO_DEST, 1, 1, 1, 1])
        self.observation_space['target'] = spaces.Box(low=low_bound, high=high_bound, shape=(self.tgt_repr_size,), dtype=np.float64)

    def set_level(self, level: str=None):
        #print("maplevel:",self.level)
        if level is None:
    ...
    def reset(self, case_id: int=None, data_dir: str=None, level: str=None) -> np.ndarray:
        ...
        self.t = 0.0
        # 重置档位奖励历史
        if hasattr(self, 'gear_shift_calc') and self.gear_shift_calc is not None:
            self.gear_shift_calc.reset()
        ...
        # 尝试获取全局图表对象
        self._try_get_plot_ax()
        
        return self.step()[0]
    
    def _try_get_plot_ax(self):
        """尝试获取全局图表对象"""
        try:
            import builtins
            if hasattr(builtins, 'global_plot_ax') and builtins.global_plot_ax is not None:
                self.current_plot_ax = builtins.global_plot_ax
                # print(f"DEBUG: 环境实例成功获取全局图表对象")
        except Exception as e:
            print(f"DEBUG: 获取图表对象失败: {e}")
    ...
    def _check_arrived(self):
        ...
        # 航向检查
        angle_diff = abs(self.vehicle.state.heading - self.map.dest.heading)
        # 处理角度环绕问题
        angle_diff = min(angle_diff, 2*np.pi - angle_diff)
        heading_ok = angle_diff < np.pi/6  # 30度容忍度

        position_ok = union_area / dest_box.area > 0.95
    ...
    def _check_status(self):
        ...
        #if self._check_action_mask():
        #    return Status.ACTION_MASK
    ...
    def _get_reward(self, prev_state: State, curr_state: State):
        ...
        # 计算车尾连线夹角
        curr_tail_line_angle_diff = self.get_tail_line_angle_diff(curr_state)
        prev_tail_line_angle_diff = self.get_tail_line_angle_diff(prev_state)
        '''print("dest_x=",self.map.dest.loc.x)
        print("dest_y=",self.map.dest.loc.y)
        print("curr_x=",curr_state.loc.x)
        print("curr_y=",curr_state.loc.y)
        print("prev_x=",prev_state.loc.x)
        print("prev_y=",prev_state.loc.y)
        print("dest_yaw=",self.map.dest.heading)
        print("curr_yaw=",curr_state.heading)
        print("prev_yaw=",prev_state.heading)

        print("curr_tail_line_angle_diff:",curr_tail_line_angle_diff)
        print("prev_tail_line_angle_diff:",prev_tail_line_angle_diff)'''
 
        needs_u_turn = False
        front_blockage_curr = 0.0
        angle_norm_ratio = math.pi

        if self.prev_action is not None and self.curr_action is not None:
            gear_shift_reward = self.gear_shift_calc.calculate(self.prev_action, self.curr_action)
        else:
            gear_shift_reward = 0

        dist_diff = curr_state.loc.distance(self.map.dest.loc)
        angle_diff = self.get_angle_diff(curr_state.heading, self.map.dest.heading)
        prev_dist_diff = prev_state.loc.distance(self.map.dest.loc)
        prev_angle_diff = self.get_angle_diff(prev_state.heading, self.map.dest.heading)
        ...
        if self.map.is_dead_end_scenario:
            prev_angle_diff = self.angle_distance(prev_state.heading, self.map.dest.heading)
            curr_angle_diff = self.angle_distance(curr_state.heading, self.map.dest.heading)
            angle_reward = (prev_angle_diff / angle_norm_ratio) - (curr_angle_diff / angle_norm_ratio)
        else:
            direction_weight = self.get_direction_weight(dist_diff, angle_diff)
            base_angle_reward = prev_angle_diff / angle_norm_ratio - angle_diff / angle_norm_ratio
            angle_reward = base_angle_reward * direction_weight
        ...
        '''print("--------------------------------")
        print(f"gear_shift_reward: {gear_shift_reward}")
        print(f"dist_reward: {dist_reward}")
        print(f"angle_reward: {angle_reward}")
        print(f"box_union_reward: {box_union_reward}")
        print(f"time_cost: {time_cost}")
        print(f"rs_dist_reward: {rs_dist_reward}")
        print("--------------------------------")'''
        return [time_cost, rs_dist_reward, dist_reward, angle_reward, box_union_reward, gear_shift_reward]
    

    def calculate_gear_shift_reward(self, prev_action, curr_action):
        pass


    def calculate_historical_gear_shift_reward(self, prev_action, curr_action):
        pass
    ...
    def get_direction_weight(self, distance_to_goal, angle_diff, M0=None, epsilon=None, k=None):
        """
        基于距离和航向夹角的方向权重函数（还原版）
        直接使用 angle_diff ∈ [0, π/2] 的正弦：M = d / (sin(|angle_diff|) + ε)
        """
        if M0 is None:
    ...
    def get_tail_line_angle_diff(self, state):
        tail_dir = self.normalize_angle(state.heading + math.pi)          # 尾向
        line_dir = math.atan2(self.map.dest.loc.y - state.loc.y, self.map.dest.loc.x - state.loc.x)      # 当前点→目标
        return self.angle_distance(tail_dir, line_dir)          # [0, π]
    def angle_distance(self, a, b):
        return abs(self.normalize_angle(a - b))            # [0, π]   

    def normalize_angle(self, a):
        return (a + math.pi) % (2*math.pi) - math.pi  # (-π, π]
    ...
    def step(self, action: np.ndarray=None):
        ...
        if action is not None:
            ...
            self.curr_action = action.copy()  # 使用copy避免引用问题
        ...
        try:
            ...
            if hasattr(self, 'current_plot_ax') and self.current_plot_ax is not None:
                if action is not None and action[1] > 0:  # 添加空值检查
                    ax = self.current_plot_ax
                    #print(f"DEBUG: 使用实例图表对象，轨迹点: ({self.vehicle.trajectory[-1].loc.x:.2f}, {self.vehicle.trajectory[-1].loc.y:.2f})")
                    ax.plot(self.vehicle.trajectory[-1].loc.x, self.vehicle.trajectory[-1].loc.y, 'ro', markersize=6, alpha=0.7)

                    current_box = self.vehicle.state.create_box()
                    box_coords = list(current_box.coords)[:-1]  # 去掉重复的最后一个点
                    box_x = [coord[0] for coord in box_coords]
                    box_y = [coord[1] for coord in box_coords]
                    #ax.plot(box_x + [box_x[0]], box_y + [box_y[0]], 'r-', linewidth=2)

                    if len(self.vehicle.trajectory) > 1:
                        traj_x = [state.loc.x for state in self.vehicle.trajectory]
                        traj_y = [state.loc.y for state in self.vehicle.trajectory]
                        ax.plot(traj_x, traj_y, 'r-', linewidth=1, alpha=0.5)
                    ...
                    ax = self.current_plot_ax
                    #print(f"DEBUG: 使用实例图表对象，轨迹点: ({self.vehicle.trajectory[-1].loc.x:.2f}, {self.vehicle.trajectory[-1].loc.y:.2f})")
                    ax.plot(self.vehicle.trajectory[-1].loc.x, self.vehicle.trajectory[-1].loc.y, 'go', markersize=6, alpha=0.7)
                    ...
                    box_x = [coord[0] for coord in box_coords]
                    box_y = [coord[1] for coord in box_coords]
                    #ax.plot(box_x + [box_x[0]], box_y + [box_y[0]], 'g-', linewidth=2)
                    
                    if len(self.vehicle.trajectory) > 1:
                        traj_x = [state.loc.x for state in self.vehicle.trajectory]
                        traj_y = [state.loc.y for state in self.vehicle.trajectory]
                        ax.plot(traj_x, traj_y, 'g-', linewidth=1, alpha=0.5)
                    #ax.legend()
                    plt.tight_layout()  
                    plt.ion()  
                    plt.show(block=False)  
    ...
    def _render(self, surface: pygame.Surface):
        ...
        for obstacle in self.map.obstacles:
            pygame.draw.polygon(surface, OBSTACLE_COLOR, self._coord_transform(obstacle.shape))
    ...
    def is_traj_valid(self, traj):
        car_coords1 = np.array(VehicleBox.coords)[:4]
        car_coords2 = np.array(VehicleBox.coords)[1:]
        car_coords_x1 = car_coords1[:, 0].reshape(1, -1)
        car_coords_y1 = car_coords1[:, 1].reshape(1, -1)
        car_coords_x2 = car_coords2[:, 0].reshape(1, -1)
        car_coords_y2 = car_coords2[:, 1].reshape(1, -1)
        vxs = np.array([t[0] for t in traj])
        vys = np.array([t[1] for t in traj])
        if np.min(vxs) < self.map.xmin or np.max(vxs) > self.map.xmax or np.min(vys) < self.map.ymin or (np.max(vys) > self.map.ymax):
            return False
        vthetas = np.array([t[2] for t in traj])
        cos_theta = np.cos(vthetas).reshape(-1, 1)
        sin_theta = np.sin(vthetas).reshape(-1, 1)
        vehicle_coords_x1 = cos_theta * car_coords_x1 - sin_theta * car_coords_y1 + vxs.reshape(-1, 1)
        vehicle_coords_y1 = sin_theta * car_coords_x1 + cos_theta * car_coords_y1 + vys.reshape(-1, 1)
        vehicle_coords_x2 = cos_theta * car_coords_x2 - sin_theta * car_coords_y2 + vxs.reshape(-1, 1)
        ...
        for obst in self.map.obstacles:
            if isinstance(obst, Area):
                obst = obst.shape
            obst_coords = np.array(obst.coords)
            if (obst_coords[:, 0] > x_max).all() or (obst_coords[:, 0] < x_min).all() or (obst_coords[:, 1] > y_max).all() or (obst_coords[:, 1] < y_min).all():
                continue
        ...
        if len(x1s) == 0:
            return True
        x1s, x2s, y1s, y2s = (np.array(x1s).reshape(1, -1), np.array(x2s).reshape(1, -1), np.array(y1s).reshape(1, -1), np.array(y2s).reshape(1, -1))
